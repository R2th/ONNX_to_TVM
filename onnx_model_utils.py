        ...
        for attr in node.attribute:
            if attr.HasField("g"):
                iterate_graph_per_graph_func(
                    attr.g, per_graph_func, **func_args)
...
def optimize_model(
    ...
    _ = ort.InferenceSession(str(model_path.resolve(strict=True)), so, providers=[
...
def make_dim_param_fixed(graph: onnx.GraphProto, param_name: str, value: int):
    ...
    """
    iterate_graph_per_graph_func(
        graph, _replace_symbolic_dim_value, dim_param=param_name, value=value)
            ...
            for idx, dim in enumerate(shape.dim):
                ...
                if dim.HasField("dim_value"):
                    ...
                    make_dim_param_fixed(
                        graph, dim.dim_param, fixed_shape[idx])
                else:
...
def _map_node_dependencies(graph: onnx.GraphProto, node_to_producers: dict, node_to_consumers: dict):
    ...
    for node in graph.node:
        ...
        for attr in node.attribute:
            if attr.HasField("g"):
                subgraph_implicit_inputs = _map_node_dependencies(
                    attr.g, node_to_producers, node_to_consumers)
        ...
        for i in inputs:
            ...
            if is_local_value(i):
                if i in producers:
                    ...
                    _create_producer_consumer_link(
                        node_to_producers, node_to_consumers, producer, node)
            else:
...
def get_producer_consumer_maps(graph: onnx.GraphProto):
    ...
    # map of node instance to nodes consuming output values it produces
    node_to_consumers = {}

    implicit_inputs = _map_node_dependencies(
        graph, node_to_producers, node_to_consumers)
